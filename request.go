...
func (client *Client) uriForAPI(api string) string {
	...
	if strings.Contains(api, "intake") {
		return url + "/intake/" + "?api_key=" +
...
func (client *Client) doJsonRequest(method, api string,
	...
	if method != "GET" && reqbody != nil {
		switch reqbody.(type) {
		case []byte:
			log.Debug("ReqBody type: []byte; sending to %s", client.uriForAPI(api))
			bjson := reqbody
			bodyreader = bytes.NewReader(bjson.([]byte))
		case Series:
			log.Debug("ReqBody type: Series; sending to %s", client.uriForAPI(api))
		...
		default:
			return fmt.Errorf("ReqBody type unknown: %s", reqbody)
		}
	}
